...
module.exports.initialize = function(isReinitialization) {
    winston.info('initialize:Initializing security. Reinitialization: ' + isReinitialization);
    var errStr = [];
    ...
    dataAccess.listSecurityRoles( function (err, roles) {
        ...
        initializeSecurityRoles(securityRoles, isReinitialization, function() {
            /*acl.allowedPermissions("110740462676845328422", "projects", function(err, permissions){
                winston.info("Daniil allows on project after initSecurityRoles: " + permissions["projects"]);
            });*/
            dataAccess.listUserRoles( null, function (err, roles) {
                if ( err ) {
                    winston.info('Initializing security: Error in giving user roles ' + err);
                    return err;
                }
...
var initializeSecurityRoles = function(securityRoles, isReinitialization, callback) {
    ...
    if(isReinitialization) {
        ...
        var afterRemoved = function(err) {
            if(err) {
                winston.info('Cannot delete "' + role + '" role.');
            }

            removedRolesCount++;
            if(removedRolesCount === securityRoles.length - 1) {
                //winston.info("All groups removed");
                initializeAllows(securityRoles, callback);
            }
        };
        for(var i = 0; i < securityRoles.length; i++) {
            //winston.info("Removing " + securityRoles[i].name);
            acl.removeRole(securityRoles[i].name, afterRemoved);
        }
    } else {
        initializeAllows(securityRoles, callback);
    }
...
var initializeAllows = function(securityRoles, callback) {
    ...
    //winston.info("all allows" + allAllowsCount);

    for (var i=0; i < securityRoles.length; i++) {
        var allowsCount = 0;
        var resources = securityRoles[i].resources;
        for (var k=0; k < resources.length; k++) {
            winston.info('initializeAllows:name=' + securityRoles[i].name + ':' + securityRoles[i]._id + ':resource=' + resources[k].name + ':permissions=' + resources[k].permissions.join(','));

            allow(securityRoles[i].name, resources[k].name, resources[k].permissions, function(err) {
                if(err) {
                    winston.info('initializeAllows:Error while allowing permissions for groups: ' + err);
                }
                allowsCount++;
                if(allowsCount === allAllowsCount) {
                    /*acl.allowedPermissions("110740462676845328422", "projects", function(err, permissions){
                    winston.info("Daniil allows on project before new allowing: " + permissions["projects"]);
                    });*/
                    winston.info('(Re-)Allowing security roles completed.');
                    callback();
                }
            });
        }
...
var createDefaultRoles = function(callback) {
    ...
    dataAccess.listSecurityRoles( function(err, body) {
        ...
        if(!employee) {
            winston.info('Creating Employee group.');
            createGroup(DEFAULT_ROLES.MINION, function(body) {
                winston.info('Employee group just created.');
                var minionGroup = { name: DEFAULT_ROLES.MINION, resource: 'securityroles/' + body.id };
                winston.info('Employee group object to insert into userRoles: { name: ' + minionGroup.name + ', resource: "' + minionGroup.resource + '" }');
                createMinionUserRoles(callback, minionGroup);
            });
        } else {
            var minionGroup = { name: DEFAULT_ROLES.MINION, resource: employee.resource };
            winston.info('Employee group already created.');
            createMinionUserRoles(callback, minionGroup);
        }
    });
};
...
var createGroup = function(name, actionAfter) {
    ...
    dataAccess.insertItem(null, group, dataAccess.SECURITY_ROLES_KEY, function(err, body){
        if (err) {
            winston.info('Error in creating default security group: ' + group.name + '. Error: ' + err);
        } else {
            if(actionAfter) {
                actionAfter(body);
            }
            winston.info('Added default security group: ' + group.name);
        }
    });
};

var createMinionUserRoles = function(callback, minionRole) {
    winston.info('Creating Employee default roles.');
    dataAccess.listPeople(null, function(err, peopleBody) {
        if(!err) {
            ...
            dataAccess.listUserRoles( null, function(err, userRolesBody) {
                ...
                var countChecked = 0;
                winston.info('People length: ' + people.length);
                ...
                for(var i = 0; i < people.length; i++) {
                    ...
                    if(!userRole) {
                        ...
                        dataAccess.insertItem(null, uRole, dataAccess.USER_ROLES_KEY, function(err, body){
                            if (err) {
                                winston.info('Error in creating default userRole: ' + err);
                            } else {
                                winston.info('Added default userRole: ' + body.id);
                            }

                            countChecked++;
                            if(callback && countChecked === people.length) {
                                winston.info('CALLBACK REACHED!');
                                callback(null, true);
                            }
                        });
                    } else {
                        if(!userRole.roles || userRole.roles.length === 0) {
                            ...
                            dataAccess.insertItem(userRole.id, userRole, dataAccess.USER_ROLES_KEY, function(err, body){
                                if (err) {
                                    winston.info('Error in updating userRole: ' + err);
                                } else {
                                    winston.info('Updated default userRole: ' + body.id);
                                }

                                countChecked++;
                                if(callback && countChecked === people.length) {
                                    winston.info('Every person has at least "Employee" permissions now.');
                                    callback(null, true);
                                }
                            });
                        } else {
                            countChecked++;
                            if(callback && countChecked === people.length) {
                                winston.info('Every person now has at least "Employee" permissions now.');
                                callback(null, true);
                            }
                        }
                    }
...
var removeRoles = function(securityRoles) {
    for(var i = 0; i < securityRoles.length; i++) {
        acl.removeRole(securityRoles[i].name, function(err) {
            if(err) {
                winston.info('Cannot delete "' + securityRoles[i].name + '" role.');
            }
        });
    }
};
...
var addRole = function(userId, roles, isReinitialization, callback) {
    ...
    if(isReinitialization) {
        acl.userRoles( userId, function(err, actualRoles) {
            ...
            winston.info("Daniil role: " + actualRoles);
            }*/
            if(!err) {

...
var cloudantSearchByKeys = function(designName, viewName, startKeys, endKeys,
	 ...
	 * Number of parameters should be equal to number of params in a key name
	 * plus one. Ex.: if you want to get data based on Project, Person, and
	 * Date, in January you need to provide keys ["ProjectPersonDate", project,
	 * person, date]: StartKeys = ["ProjectPersonDate",
	 * "projects/52aba189e4b0fd2a8d13002e", "people/52ab7005e4b0fd2a8d130017",
	 * "2014-01-01"] EndKeys = ["ProjectPersonDate",
	 * "projects/52aba189e4b0fd2a8d13002e", "people/52ab7005e4b0fd2a8d130017",
	 * "2014-01-31"]
	 * 
	 * Current valid key Names for Hours records: "Person"; "PersonDate";
	 * "PersonProject"; "PersonProjectDate"; "ProjectPerson"; "PersonDate";
	 * "ProjectPersonDate"; "DateProjectPerson"; "DatePersonProject";
	 * "DatePerson"; "DateProject";
	 */
	...
	if (startKeys && endKeys)
		query = '{ "queries" : [ {"startkey" :  ' + JSON.stringify(startKeys)
				+ ', "endkey" :    ' + JSON.stringify(endKeys)
				+ ', "include_docs" : true}]}';
	else {
		//query = JSON.stringify({queries: [{keys: startKeys, include_docs: true}]});
		// query = JSON.stringify({keys: startKeys});
		query = '{"keys": ' + JSON.stringify(startKeys) + '}';
		extraParam = '?include_docs=true';
	}
	
	nano.request({
		db : database,
		method : 'post',
		path : '/_design/' + designName + '/_view/' + viewName + extraParam,
...
var prepareResponse = function(data, about, valProp) {
	...
	if (data) {
		result.data = _.map(data.rows, function(val, key) {
			return val[valProp]
		});
	}
...
// startKeys = ["ProjectPersonDate", "projects/52aba189e4b0fd2a8d13002e",
// "people/52ab7005e4b0fd2a8d130017", "2014-01-01"]
// endKeys = ["ProjectPersonDate", "projects/52aba189e4b0fd2a8d13002e",
// "people/52ab7005e4b0fd2a8d130017", "2014-01-31"]

module.exports.listHoursByStartEndDates = function(start, end, callback) {
	cloudantSearchByKeys('views', 'AllHoursInOne', start, end, function(err,
			body) {
		callback(err, prepareResponse(
				body && body.results.length == 1 ? body.results[0] : {},
				'hours', 'doc'));
		// callback(err, body);
	});
};
...
module.exports.listHoursByProjects = function(projects, callback) {
	...
	cloudantSearchByKeys('views', 'AllHoursInOne', processedProjects, null,
			function(err, body) {
				var rows = body && body.results && body.results.length == 1 ? body.results[0] : null;
				
				rows = rows ? rows: body;
				
				callback(err, prepareResponse(rows ? rows : {}, 'hours', 'doc'));
				// callback(err, body);
			});
};
...
/*
 * module.exports.listHours = function(q, callback) { var start =
 * ["ProjectPersonDate", "projects/52aba189e4b0fd2a8d13002e",
 * "people/52ab7005e4b0fd2a8d130017", "2014-01-01"]; var end =
 * ["ProjectPersonDate", "projects/52aba189e4b0fd2a8d13002e",
 * "people/52ab7005e4b0fd2a8d130017", "2014-01-01"];
 * 
 * cloudantSearchByKeys('views', 'AllHoursInOne', start, end, function(err,
 * body){ callback(err, prepareResponse(body.results.length == 1 ?
 * body.results[0]: {}, 'hours', 'doc')); //callback(err, body); }); };
 */
...
module.exports.listSecurityRoles = function(callback) {
	cloudantView('views', 'SecurityRoles', {
		include_docs : true
	}, function(err, body) {
...
module.exports.listConfiguration = function(callback) {
	cloudantView('views', 'Configuration', {
		include_docs : true
	}, function(err, body) {
...
module.exports.listNotifications = function(callback) {
	cloudantView('views', 'Notifications', {
		include_docs : true
	}, function(err, body) {

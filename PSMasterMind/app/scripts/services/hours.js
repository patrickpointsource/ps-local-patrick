...
angular.module('Mastermind')
  .factory('Hours', ['$q','Restangular', 'Resources', 'RolesService', function ($q, Restangular, Resources, RolesService) {
    ...
    var getQueryDateOneMonthAgo = function(date){
    	...
    	oneMonthBeforeToday.setMonth(date.getMonth() - 1);
	      ...
	      var mm1 = oneMonthBeforeToday.getMonth()-1; //January is 0!
    ...
    function getTaskHours(){
    	...
    	Resources.query('assignments', assignmentsQuery, {}, function(result){
    		...
    		for(var i = 0; i < projectAssignments.length; i++){
    			...
    			for(var j = 0; j < projectAssignment.members.length;j++){
    				...
    				if (assignment.person && assignment.person.resource &&
    		                activeHours.indexOf(assignment.person.resource) === -1){
    		...
    		var pepInRolesQuery = {_id:{$nin:activeHours},'primaryRole.resource':{$exists:true}};
...
//            		}
        	...
        	if(hoursIds.length <= 0){
        		...
        		//Fetch all the hours
        		var pepInRolesQuery = {_id:{$in:hoursIds}};
    	        var pepInRolesFields = {resource:1,name:1,familyName:1,givenName:1,primaryRole:1,thumbnail:1};
    	        Resources.query('hours',pepInRolesQuery,pepInRolesFields,function(result){

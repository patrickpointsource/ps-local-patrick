	  ...
	  this.getCurrentPersonProjects = function(person){
		  ...
		  Resources.query('assignments', query, fields).then(function(result){
	        	...
	        	for (var  i = 0; i < projectAssignments.length; i ++) {
	        		...
	        		projectOIDs.push({$oid: resource.substring(resource.lastIndexOf('/')+1)});
	        	}
	        	
	        	 var projectsQuery = { _id: { $in:projectOIDs }};
	        	 var projectFields = {resource:1,name:1,startDate:1,endDate:1,'roles':1,customerName:1,committed:1,type:1,description: 1};

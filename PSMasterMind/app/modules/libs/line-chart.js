    ...
    .factory('n3utils', ['$window', function ($window) {
        return {
            ...
            drawArea: function (svg, scales, data, interpolateMode) {
                var areaSeries = data.filter(function (series) {
                    return series.type === 'area';
                });

                areaSeries.forEach(function (series) {
                    this.addPattern(svg, series);
                }, this);
            ...
            drawDots: function (svg, axes, data) {
                ...
                svg.select('.content').selectAll('.dotGroup')
                    .data(data.filter(function (s) {
                        return s.type === 'line' || s.type === 'area';
                    }))
                    .enter().append('g')
                    .attr('class', function (s) {
                ...
                item.append('path')
                    .attr({
                        'fill-opacity': function (s) {
                            return (s.type === 'area' || s.type === 'column') ? '1' : '0';
                        },
                        'fill': 'white',
                        'stroke': 'white',
                        'stroke-width': '2px',
            ...
            setScalesDomain: function (scales, data, series, svg, axesOptions) {
                ...
                var ySeries = series.filter(function (s) {
                    return s.axis !== 'y2';
                });
                var y2Series = series.filter(function (s) {
                    return s.axis === 'y2';
                });

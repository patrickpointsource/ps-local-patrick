    ...
    .service('ProjectsService', ['$q', '$sanitize', 'Restangular', 'Resources', 'Project', 'HoursService', '$injector', '$timeout',
        ...
        this.save = function (project) {
            ...
            if (this.isTransient(project)) {
                resultPromise = Resources.create('projects', project);
                ...
                resultPromise = Resources.update(project);
                /*.then(function(result) {
                 deferred.resolve(result);
                 });*/
            }
            ...
            Resources.get("roles").then(function (roles) {
                ...
                Resources.get("people/bytypes/active", params).then(function (result) {
                    ...
                    for (var k = 0; k < result.members.length; k++) {
                        utilizationRate = 100;
                        roleType = _.find(roleList, function (r) {
                            return result.members[k].primaryRole && result.members[k].primaryRole.resource && result.members[k].primaryRole.resource.indexOf(r.resource) > -1;
                        });

...
function ($scope, $q, $state, $stateParams, $filter, Resources, AssignmentService, ProjectsService, TasksService, RolesService, HoursService, People, $rootScope, TableParams)
{
			...
			TasksService.refreshTasks( ).then( function( tasks ) {
				...
				$( 'input[name="reportProject"]' ).on( 'typeahead:selected', function( e, selected ) {
					$scope.$apply( function( ) {
						...
						$('input[name="reportClient"]').typeahead('val', '');
					} );
				} );
	...
	$scope.processAndApplyFilters = function( ) {
		...
		var reportClient = $( 'input[name="reportClient"]' ).val();
		var reportPerson = $( 'input[name="reportPerson"]' ).val();
		var reportProject = $( 'input[name="reportProject"]' ).val();
	...
	$scope.getHoursReportData = function( cb ) {
		...
		AssignmentService.getAssignments( result ).then( function( assignments ) {
			...
			if (reportPerson) {
				var hoursQ = {
						'person.resource': reportPerson.resource
					};
				var fields = {};
				HoursService.query(hoursQ, fields).then(function( hoursResult ) {
					$scope.prepareHoursReportData(hoursResult.members, filtered, cb);
				});
			} else {
				var hoursQ = {
						$or: [ ]
	...
	$scope.prepareHoursReportData = function( reportHours, filter, cb ) {
		...
		var findPersonOnProject = function( rolesPersonMapping, resource ) {
			...
			for( prop in rolesPersonMapping ) {
				if (prop != CONSTS.UNKNOWN_ROLE || $scope.userRoles[ 'all' ].value) { //Show Unassigned records if 'All roles' selected ONLY.
		...
		var reportClient = filter.reportClient;
		var reportProject = filter.reportProject;
		var reportPerson = filter.reportPerson;
		var projectMapping = filter.projectMapping;
				...
				if( $scope.activeTab[ 'hours' ] ) {
					...
					for( k = 0; record.roles[ j ].persons && k < record.roles[ j ].persons.length; k++ ) {
						...
						if( !record.roles[ j ].persons[ k ].hours || record.roles[ j ].persons[ k ].hours.length == 0 ) {
							//Does not show line if hours absent. (By Brian confirm)
							continue;
							
//							line += $scope.hoursToCSV.stringify( record.name ) + ',';
//							line += $scope.hoursToCSV.stringify( record.roles[ j ].persons[ k ].name ) + ',';
			...
			if( !record.roles )
				for( k = 0; record.persons && k < record.persons.length; k++ ) {
					...
					for( l = 0; record.persons[ k ].hours && l < record.persons[ k ].hours.length; l++ ) {
						...
						line += $scope.hoursToCSV.stringify( record.name ) + ',';
						line += '--,';
						line += $scope.hoursToCSV.stringify( record.persons[ k ].name ) + ',';

...
angular.module('Mastermind.controllers.reports').controller('ProjectReportCtrl', ['$scope', '$rootScope', '$q', '$state', '$stateParams', '$filter', '$location', '$anchorScroll', 'AssignmentService', 'ProjectsService', 'ReportExportService', 'Resources', '$timeout', '$interval',
    function ($scope, $rootScope, $q, $state, $stateParams, $filter, $location, $anchorScroll, AssignmentService, ProjectsService, ReportExportService, Resources, $timeout, $interval) {
            ...
            AssignmentService.getAssignments(projects || []).then(function (data) {
                ...
                for (var i = 0, projCount = data.length; i < projCount; i++)
                    for (var j = 0, assignmentCount = data[i].members.length; j < assignmentCount; j++) {
                        ...
                        var assignee = _.find($scope.peoples, function (p) {
                            return p.resource == assignment.person.resource;
                        });

                        if (!assignee)
                            continue;

                        var roleName = _.find($scope.roles, function (r) {
                            return assignee.primaryRole && r.resource == assignee.primaryRole.resource;
                        });
                ...
                $scope.assignedRoles = _.sortBy(assignedRoles, function (ar) {
                    return ar;
                });
            });
        }
        ...
        $scope.selectAllAssignmentHours = function (selected) {
            ...
            _.each($scope.assignedRoles, function (ar) {
                $scope.params.fields.selectedAssignedRoles[ar] = selected;
            });
        ...
        $scope.prepareInputParams = function () {
            ...
            _.each(projectList, function (p) {
                input.projects.push({resource: p.resource});
            });
            ...
            $scope.generationTimer = $interval(function () {
                    ...
                    if (timer) {
                        ...
                        var spentTime = moment.utc(moment(now, "DD/MM/YYYY HH:mm:ss")
                            .diff(moment($rootScope.projectReportGenerationStartTime, "DD/MM/YYYY HH:mm:ss")))
                            .format("HH:mm:ss");
        ...
        $scope.cancelReportGeneration = function () {
            ...
            $rootScope.projectReportGenerationStartTime = null;

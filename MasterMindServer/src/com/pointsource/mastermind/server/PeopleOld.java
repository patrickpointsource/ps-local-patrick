 ...
 * @author kmbauer
 */
@Path("/" + CONSTS.RESOURCE_PEOPLE_OLD)
@Workspace(workspaceTitle = CONSTS.WORKSPACE_TITLE, collectionTitle = CONSTS.RESOURCE_TITLE_PEOPLE_OLD)
public class PeopleOld extends BaseResource {
	/**
	...
	public Response get() {
		try {
			try {
				...
				JSONObject ret = Data.getPeopleOld(context);
	...
	public Response getPerson(@PathParam("id") String id) {
		try {
			try {
				RequestContext context = getRequestContext();
				JSONObject ret = Data.getPersonOld(context, id);
				return Response.ok(ret).build();
			} catch (WebApplicationException e) {
				return handleWebApplicationException(e);
			} catch (Exception e) {
